...
const tabList = ["Lux Rpc Status", "Lux Data Service"];
...
export const NodeModal = (props: Modal["props"] & TargetChain) => {
  ...
  const arr = useMemo(() => {
    return activeTab === 0 ? data?.lux_rpc : data?.lux_data_service;
  }, [activeTab, data]);
  return (
    <Modal
      ariaHideApp={false}
...
const tips: Record<string, string> = {
  ...
  "Lux Data Service":
    "Lux's backend data service. Unstable status may cause delays in updating asset balances and transaction records.",
};
      ...
      >
        ...
        >
          ...
          <Line
            ...
            name={
              isDataService
                ? "Lux's data synchronization level"
                : "Lux's RPC block height"
            }
          />
        </LineChart>
      </div>
